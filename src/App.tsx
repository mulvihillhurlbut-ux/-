...
export default function App() {
  ...
  const [timeRemaining, setTimeRemaining] = useState<number>(600); // 10 minutes (600 seconds)
  ...
  const getTimelineStoryLine = () => {
    if (timeRemaining > 450) {
      return {
        title: "第一章：开局降临 · 契约初铸",
        desc: "殷墟圣坛之上，青铜礼器嗡鸣作响。10位商周精灵已按远古契约分列到位，其中暗伏着3名来自文创的杂染卧底。好人们全程迷失自我，摸索天机。信封灵气充沛，尽速破解字谜获取甲骨吧！"
      };
    } else if (timeRemaining > 300) {
      return {
        title: "第二章：破晓探秘 · 拨雾寻踪",
        desc: "神兽密室开启，古圣雕像俯瞰凡尘。随着第一轮三人核验与密室叩问，身份候选初露端倪。卧底也开始了危险的暗中窥伺，密室四周泛起阵阵冷雾，切记护好真守法身！"
      };
    } else if (timeRemaining > 120) {
      return {
        title: "第三章：对决胶着 · 裂纹问天",
        desc: "对局愈加胶着，每个人都带着半遮的面具，在真伪的两端权衡。卧底开始消耗甲骨频繁向好人发作致命诅咒。天命盘旋，大祭司的钟声正在倒数，留给好人填报自身神兽的时间不多了！"
      };
    } else if (timeRemaining > 0) {
      return {
        title: "第四章：终局决战 · 乾坤祭天",
        desc: "最后的钟声！信封灵气即将消耗殆尽。所有生还的守护精灵必须速速在终极验证台各归其位。差错一人，或是混入任何一个邪恶文创卧底，整个商周社稷就将沦陷！"
      };
    } else {
      return {
        title: "超时：灵气散尽 · 虚无暗寂",
        desc: "10分钟限时已过！天地灵气彻底散去，信封字谜不复刷新，祭坛重光凋零。只能依靠现存的甲骨余温竭力一试了！"
      };
    }
  };
...
`;
    ...
    const initialLogs: GameEventLog[] = [
      {
        ...
        timestamp: nowStr,
        sender: '系统',
        message: `殷商神坛大门洞开，${userName} 与其他 9 位商周豪杰齐入圣坛！10分钟倒计时已启动！`,
        type: 'system'
      },
      {
        id: 'start_sys_2',
    ...
    const interval = setInterval(() => {
      setTimeRemaining(prev => {
        if (prev <= 1) {
          ...
          addLog('系统', '限时10分钟已届满！信封不再刷新，空间开始萎缩。请尽快校验终极验证！', 'system');
          return 0;
        }
        return prev - 1;
      });
  ...
  const handlePickupEnvelope = () => {
    if (timeRemaining <= 0) {
      alert("限时10分钟已过！天地灵气消散，不再刷新新的信封。请尽快进行组队核验或终局校验！");
      return;
    }
  ...
  const executeOneRandomAIMove = () => {
    ...
    if (bot.isGood) {
      ...
      if (timeRemaining > 0) {
        ...
        if (coin < 0.6) {
          ...
          const phrases = [
            ...
            "大家抓紧时间把字谜做起来，10分钟倒计时一完就可以去神秘阁验证啦。"
          ];
      ...
      if (timeRemaining > 0) {
        ...
        // After timeRemaining === 0, Undercover bot can probe or guess kill!
        const coin = Math.random();
        if (!bot.hasEnteredChamber) {
          ...
          {hasStarted && (
            <div className="flex flex-wrap items-center justify-center gap-3">
              ...
              <div className="flex items-center space-x-2 bg-stone-900/90 border border-stone-800 px-3 py-1.5 rounded-xl">
                ...
                <span className="text-[9px] text-stone-500 border-l border-stone-800 pl-2">限时解谜</span>
                {timeRemaining > 0 && (
                  <button
                    onClick={() => {
                      if (confirm("确定要直接结束十分钟解谜期，进入后续神密阁印章与组队校验阶段吗？")) {
                        setTimeRemaining(0);
                        addLog('大祭司', '祭司动用神符奥义，提前结束了限时解谜阶段！神秘阁的古门缓缓开启...', 'system');
                        handlePlayBell(880);
                      }
                    }}
                    className="ml-2 px-1.5 py-0.5 rounded bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 transition text-[9px] font-bold cursor-pointer border border-amber-500/30"
                    title="提前结束限时解谜并开启神秘阁"
                  >
                    ⏩ 进神秘阁
                  </button>
                )}
            ...
            <div className="lg:col-span-8 flex flex-col justify-between space-y-5">
              ...
              <nav className="flex justify-around items-center gap-1.5 p-1 bg-stone-900/90 border border-stone-800 rounded-2xl md:p-2 shrink-0">
                ...
                <button
                  onClick={() => {
                    if (timeRemaining > 0) {
                      alert("⚠️ 神秘阁大门尚未开启！必须在猜谜十分钟结束后，才允许去神秘阁进行印章验证！");
                      return;
                    }
                ...
                >
                  {timeRemaining > 0 ? <Lock className="w-3.5 h-3.5 text-stone-500" /> : <Compass className="w-3.5 h-3.5" />}
                  <span>{timeRemaining > 0 ? '神秘阁 [锁]' : '神密阁.印章'}</span>
                </button>

                <button
                  onClick={() => {
                    if (!localPlayer?.hasEnteredChamber) {
                      alert("⚠️ 誓言不配！你必须先去「神秘阁.印章」拜叩神兽进行印章验证后，才能开启三人组队核验！");
                      return;
                    }
                    setActiveTab('check');
                    handlePlayWhoosh();
                  ...
                  } ${!localPlayer?.hasEnteredChamber ? 'opacity-50 cursor-not-allowed font-medium' : ''}`}
                >
                  {!localPlayer?.hasEnteredChamber ? <Lock className="w-3.5 h-3.5 text-stone-500" /> : <Users className="w-3.5 h-3.5" />}
                  <span>{!localPlayer?.hasEnteredChamber ? '组队核验 [锁]' : '组队核.研契'}</span>
                </button>
                ...
                <button
                  ...
                  className={`flex-1 py-1.5 text-[10px] md:text-xxs uppercase tracking-wider font-bold transition rounded-xl flex items-center justify-center space-x-1 duration-200 cursor-pointer ${
                    activeTab === 'final' ? 'bg-amber-600 border border-amber-400 text-stone-950' : 'text-stone-400 hover:bg-stone-850 hover:text-stone-200'
                  }`}
                >
                          ...
                          return (
                            <div key={p.id} className={`perspective-1000 w-full h-[225px] select-none group transition-all duration-300 ${isTracked ? 'scale-[1.04] ring-2 ring-amber-500 shadow-[0_0_25px_rgba(245,158,11,0.5)] rounded-2xl z-20' : ''}`}>
                              <div className={`relative w-full h-full duration-500 preserve-3d transition-transform ${isFlipped ? 'rotateY-180' : ''}`}>
                                ...
                                <div className={`absolute inset-0 backface-hidden flex flex-col justify-between p-3 rounded-2xl border bg-stone-950/95 shadow-[0_4px_20px_rgba(0,0,0,0.65)] duration-300 overflow-hidden ${isTracked ? 'border-amber-400 bg-amber-955/20' : 'border-amber-500/25 hover:border-amber-500/60'}`}>
                                ...
                                <div className="absolute inset-0 backface-hidden rotateY-180 rounded-2xl overflow-hidden p-[1.5px] shadow-[0_0_20px_rgba(245,158,11,0.6)]">
                                      ...
                                      {chamberResultOptions.map((beast, idx) => {
                                        ...
                                        return (
                                          <button
                                            ...
                                            className="px-4 py-2 bg-gradient-to-b from-stone-900 to-stone-950 border border-amber-600/30 hover:border-teal-400 hover:bg-stone-900 rounded-xl flex flex-col items-center justify-between text-center min-w-20 transition-all duration-100 hover:scale-105 active:scale-95 cursor-pointer shadow"
                                          >
                    ...
                    <div>
                      ...
                      <div className="bg-stone-950 p-4 rounded-xl border-2 border-amber-600/20 space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {players.map(p => (
                            <div key={p.id} className={`p-2.5 rounded border flex items-center justify-between ${p.isEliminated ? 'bg-stone-900/30 border-stone-900/40 opacity-75' : 'bg-stone-900/60 border-stone-800'}`}>
                        ...
                        {/* SUBMIT BIG GOLD COMPONENT */}
                        <div className="border-t border-stone-900 pt-3 text-center">
                          <button
                            onClick={handleFinalVerifySubmission}
                            className="px-8 py-3 rounded-full bg-gradient-to-r from-amber-500 to-amber-700 text-stone-950 font-black text-sm tracking-widest hover:scale-105 transition shadow-xl cursor-pointer"
                          >
                    ...
                    {players.map(p => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            onClick={() => handleToggleBambooStamp(p.id, beastSymbol)}
                            className={`w-full mt-2 py-1 text-[8.5px] font-serif font-black rounded-lg border transition-all duration-100 hover:scale-[1.03] active:scale-95 cursor-pointer flex items-center justify-center gap-0.5 ${
                              isStamped
